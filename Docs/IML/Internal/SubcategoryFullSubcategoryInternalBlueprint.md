...
The `syntax_def` admissions are part of the problem. Later proofs need their checked bodies to
expose the relevant factorization data.
...
Each package should say that a map or collection has a concrete factorization property. For
example, preserving a morphism collection means that an induced map on arrow anima factors through
that collection. The admitted bodies do not provide checked projections or constructors for obvious
cases such as "the total morphism collection contains identities".
...
Axiom H constructs the subcategory of `C` generated by the selected morphisms, together with the
universal property of maps that preserve those morphisms. The package below has the category, the
inclusion, preservation evidence, and lift data. It omits the proof that the inclusion is a
subcategory:
...
Because the package omits the subcategory witness, the internal admission

```lean
subcategoryInclWitness
```

has no obvious projection source. The repair is to store the witness in the same package, next to
the category and inclusion it refers to:
...
After this change, the basic projections have a clear meaning:
...
This changes the model interface by adding data that Axiom H already promises: the constructed
inclusion is a subcategory inclusion. The quasicategory model will eventually have to provide this
witness anyway.
