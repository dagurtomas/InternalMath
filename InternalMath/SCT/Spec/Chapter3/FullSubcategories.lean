...
/-!
# Chapter 3 full subcategories from object collections

This file builds full subcategories from object collections using Axiom H.

Book guide, paraphrasing the May 2026 draft:

- Given an object collection `P ⊆ C^≃`, the full subcategory on `P` is generated by the morphisms
  whose source and target lie in `P`.
- `EndpointRestrictedMorphismData` packages endpoint membership for an interval-shaped arrow.
- `fullSubcategoryMorphismComprehensionPackage` stores the endpoint-restricted morphism collection,
  its identity and composition closure evidence, and the two directions relating membership to
  endpoint data.
- The projections `fullSubcategoryMorphismCollection`,
  `fullSubcategoryMorphismContainsIdentities`, and `fullSubcategoryMorphismClosed` feed Axiom H.
- `fullSubcategory`, `fullSubcategoryIncl`, and `fullSubcategoryInclWitness` are the resulting
  category, inclusion, and subcategory witness.
- `fullSubcategoryCoreEquiv` and `fullSubcategoryAllObjectsEquiv` are theorem debt about the core
  and the all-objects case.
-/
